...
// reInitMu serializes ReInit calls. SetSampleRate must NOT be called while
// holding mu, because the PulseAudio callback (c.read) takes mu and would
// deadlock against the ReInit caller.
var reInitMu sync.Mutex

// ReInit dynamically changes the speaker's sample rate. The underlying audio
// context and player are recreated with the new sample rate so that the
// player's buffer size is consistent with the new rate. If the speaker is not
...
func ReInit(sampleRate beep.SampleRate, bufferSize int) error {
	...
	if context == nil {
		mu.Unlock()
		return Init(sampleRate, bufferSize)
	}
